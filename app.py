...
def class_feature_tooltips(features, inline_descriptions=None):
    ...
    for feature in dict.fromkeys(features or []):
        ...
        if feature.startswith("Skill Proficiency: "):
            proficiency_skill = feature.split(":", 1)[1].strip()
            proficiency_description = f"Add your proficiency bonus to {proficiency_skill} checks."
...
SUBCLASS_SPELLCASTERS = {
    ("Fighter", "Eldritch Knight"): {
        "spell_list": "Eldritch Knight",
        "ability": "Intelligence",
        "cantrips": [0, 0, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3],
        "learned": [0, 0, 3, 4, 4, 4, 6, 7, 7, 8, 9, 9],
        "slots": [[0, 0], [0, 0], [2, 0], [3, 0], [3, 0], [3, 0], [4, 2], [4, 2], [4, 2], [4, 3], [4, 3], [4, 3]],
    },
    ("Rogue", "Arcane Trickster"): {
        "spell_list": "Arcane Trickster",
        "ability": "Intelligence",
        "cantrips": [0, 0, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3],
        "learned": [0, 0, 3, 4, 4, 4, 5, 6, 6, 7, 8, 8],
        "slots": [[0, 0], [0, 0], [2, 0], [3, 0], [3, 0], [3, 0], [4, 2], [4, 2], [4, 2], [6, 2], [6, 2], [6, 2]],
    },
}


def spell_profile(class_name: str, class_level: int, ability_data, feat_effects, equipment_effects=None,
                  subclass: str | None = None) -> dict[str, Any] | None:
    subclass_profile = SUBCLASS_SPELLCASTERS.get((class_name, subclass))
    if (class_name not in PREPARED_CASTERS | KNOWN_CASTERS and not subclass_profile) or class_level < 1:
        return None
    row = CLASS_PROGRESSIONS[class_name][class_level - 1]
    if subclass_profile:
        slot_counts = subclass_profile["slots"][class_level - 1]
        max_spell_level = max((level for level, count in enumerate(slot_counts, 1) if count), default=0)
        cantrip_limit = subclass_profile["cantrips"][class_level - 1]
        learned_limit = subclass_profile["learned"][class_level - 1]
        ability = subclass_profile["ability"]
        spell_list = subclass_profile["spell_list"]
    else:
        slot_counts = [numeric_progression_value(row, f"spell_slots_{level}") for level in range(1, 7)]
        max_spell_level = max((level for level, count in enumerate(slot_counts, 1) if count), default=0)
    ...
    return {
        ...
        "spell_list": spell_list, "slots": slot_counts, "known_caster": bool(subclass_profile) or class_name in KNOWN_CASTERS,
    }
...
@callback(
    ...
    Output({"type": "level-feat", "level": ALL}, "value", allow_duplicate=True),
    Input({"type": "level-class", "level": ALL}, "value"),
    State({"type": "level-subclass", "level": ALL}, "value"),
    State({"type": "level-feat", "level": ALL}, "value"),
    State("pending-build-load", "data"),
    prevent_initial_call=True,
)
def clear_invalid_level_chain(class_values, subclass_values, feat_values, pending_build):
    """Discard controls whose character level no longer exists or whose class changed."""
    if pending_build:
        return no_update, no_update, no_update
    classes = (list(class_values or []) + [None] * 12)[:12]
    subclasses = (list(subclass_values or []) + [None] * 12)[:12]
    feats = (list(feat_values or []) + [None] * 12)[:12]
    triggered = ctx.triggered_id
    changed_level = triggered.get("level") if isinstance(triggered, dict) else None

    first_gap = next((index for index, value in enumerate(classes) if not value), 12)
    normalized_classes = classes[:first_gap] + [None] * (12 - first_gap)
    normalized_subclasses = list(subclasses)
    normalized_feats = list(feats)
    for index in range(first_gap, 12):
        normalized_subclasses[index] = None
        normalized_feats[index] = None

    # A replacement class changes all subsequent per-class level counts, so
    # subclass, feat, expertise, fighting-style, and similar choices must be
    # rebuilt from the changed row onward.
    if changed_level:
        for index in range(changed_level - 1, 12):
            normalized_subclasses[index] = None
            normalized_feats[index] = None

    return (
        normalized_classes if normalized_classes != classes else no_update,
        normalized_subclasses if normalized_subclasses != subclasses else no_update,
        normalized_feats if normalized_feats != feats else no_update,
    )
...
@callback(
    ...
    Input("background-dropdown", "value"), Input("race-dropdown", "value"),
    Input("subrace-dropdown", "value"), Input("human-versatility-skill", "value"),
    Input("feat-effects-store", "data"), Input("skill-state-store", "data"),
    State({"type": "class-feature-choice", "level": ALL, "feature": ALL}, "id"),
)
def render_class_feature_choices(class_values, subclass_values, current_values, background, race, subrace,
                                 human_skill, feat_effects, skill_state, current_ids):
    counts = Counter()
    choices = []
    chosen_subclasses = {}
    ...
    proficient_skills = skills_mentioned(
    ...
    if race == "Human" and human_skill:
        proficient_skills.add(human_skill)
    ...
    for (choice_level, feature), value in existing.items():
        chosen = value if isinstance(value, list) else [value]
        if feature == "Skill Proficiencies":
            proficient_skills.update(skill for skill in chosen if skill in SKILL_TO_ABILITY)
        elif feature in {"Favoured Enemy", "Natural Explorer"}:
            proficient_skills.update(ranger_skill_grants[choice] for choice in chosen if choice in ranger_skill_grants)
        elif feature == "Knowledge of the Ages":
    ...
    def choice_control(level, feature, label, values, multi=False, limit=None, unique=False, valid_only=False):
        ...
        if valid_only:
            allowed = set(values)
            current = [item for item in (current or []) if item in allowed][:limit] if multi else current if current in allowed else None
        ...
        return html.Div([
            ...
            dcc.Dropdown(
                ...
                optionHeight=105, maxHeight=390,
            ),
    ...
    for level, class_name in enumerate(class_values or [], 1):
        ...
        if counts[class_name] == 1:
            ...
            if skill_limit and skill_options:
                controls.append(choice_control(
                    ...
                    sorted(skill_options), multi=True, limit=skill_limit, unique=True, valid_only=True,
                ))
        ...
        if ((class_name == "Rogue" and class_level in {1, 6})
            ...
            controls.append(choice_control(
                level, "Expertise", "Expertise skills (2)", sorted(proficient_skills),
                multi=True, limit=2, unique=True, valid_only=True,
            ))
        if class_name == "Bard" and class_level == 3 and subclass == "College of Lore":
            controls.append(choice_control(
                level, "Skill Proficiencies", "College of Lore Skills (3)", sorted(SKILL_TO_ABILITY),
                multi=True, limit=3, unique=True, valid_only=True,
            ))
        if class_name == "Cleric" and class_level == 2 and subclass == "Knowledge Domain":
            controls.append(choice_control(
                level, "Knowledge of the Ages", "Knowledge of the Ages ability", ABILITIES,
            ))
...
def render_sheet_leveling(class_values, subclass_values, feat_values, ability_data, feat_effects, skill_state, equipment_effects, class_choice_values, class_choice_ids):
    ...
    for value, item_id in zip(class_choice_values or [], class_choice_ids or []):
        ...
        if source_class and value:
            ...
            elif category in {"Knowledge of the Ages", "Astral Knowledge"}:
                displayed = [f"{category}: {choice}" for choice in chosen]
            else:
                displayed = chosen
...
def render_skills(ability_data, background, race, subrace, human_skill, level_classes, level_subclasses,
    ...
    for value, item_id in zip(class_choice_values or [], class_choice_ids or []):
        if item_id.get("feature") == "Skill Proficiencies":
            proficient.update(value if isinstance(value, list) else [value])
            continue
        ...
        if item_id.get("feature") in {"Knowledge of the Ages", "Astral Knowledge"}:
            for ability in (value if isinstance(value, list) else [value]):
                proficient.update(skill for skill, skill_ability in SKILL_TO_ABILITY.items() if skill_ability == ability)
            continue
    ...
    for skill in sorted(SKILL_TO_ABILITY):
        ...
        jack_of_all_trades = (level_classes or []).count("Bard") >= 2
...
def render_spell_builder(level_classes, level_subclasses, ability_data, feat_effects, equipment_effects):
    ...
    for class_name, subclass in zip(level_classes or [], level_subclasses or []):
        if class_name and subclass:
            chosen_subclasses[class_name] = subclass
    cards = []
    for class_name, class_level in counts.items():
        ...
        if not profile or (not profile["cantrips"] and not profile["max_spell_level"]):
            continue
        class_spells = [row for row in SPELLS if profile["spell_list"] in row["classes"].split("; ")]
        ...
        if profile["known_caster"] and profile["learned"] and levelled_options:
        ...
        slots = ", ".join(
            f"L{level}: {count}"
            for level, count in enumerate(profile["slots"], 1)
            if count
        )
        cards.append(
            html.Section(
...
def class_granted_spells(level_classes, level_subclasses, class_choice_values):
    ...
    if ("Rogue", "Arcane Trickster") in selected_subclasses:
        granted.setdefault("Rogue", []).append("Mage Hand")
...
def filter_owned_spell_options(values, level_classes, level_subclasses, class_choice_values, race, subrace, ids):
    ...
    for value, item_id in zip(values, ids):
        ...
        profile = spell_profile(class_name, class_level, {}, {}, {}, chosen_subclasses.get(class_name))
        max_spell_level = profile["max_spell_level"] if profile else 0
        spell_list = profile["spell_list"] if profile else class_name
        class_spells = [row for row in SPELLS if spell_list in row["classes"].split("; ")]
        if kind == "cantrips":
